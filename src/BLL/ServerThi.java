 ...
 */
public class ServerThi implements Runnable {

    private Socket socket;
    private String name;

    public ServerThi(Socket socket, String name) throws IOException {
        this.socket = socket;
        this.name = name;
        ...
        try {
            ...
            switch (Integer.parseInt(receiveArray[0])) {
                ...
                case 5:
                    ...
                    {
                        try {
                            if (ConnectDB.CheckLogIn(arrStr1[1], arrStr1[2]) == 1) {
                                dos.writeUTF("Connected");
                            } else {
                                dos.writeUTF("Connected fail");
                            }
                        } catch (SQLException ex) {
                            Logger.getLogger(ServerThi.class.getName()).log(Level.SEVERE, null, ex);
                        }                   
                    } 
                    break;
            }
